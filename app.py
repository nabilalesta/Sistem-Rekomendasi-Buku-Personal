...
def get_recommendations(username, filtered_indices=None, top_n=5):
    ...
    """
    # Pastikan user_profiles sudah dimuat
    if not user_profiles:
        load_all_models()
    
    # Gunakan profil pengguna jika ada, jika tidak buat profil default
    if username in user_profiles:
        ...
        if 'default' not in user_profiles:
            ...
            # Simpan profil default
            with open('user_profiles.pkl', 'wb') as f:
                pickle.dump(user_profiles, f)
            # Simpan juga ke ModelRekomendasi.pkl
            save_all_models()
...
def show_home_page():
    ...
    # Jika user sudah login, tampilkan buku-buku yang ada di profil
    ...
    with tab1:
        ...
        if st.button("Simpan Preferensi"):
            if st.session_state.username in user_data:
                # Simpan preferensi ke user_data
                ...
                st.rerun()  # Refresh halaman untuk menampilkan rekomendasi baru
    
    with tab2:
        st.subheader("Rekomendasi Buku")
        
        # Tampilkan rekomendasi secara otomatis jika ada preferensi
        if st.session_state.username in user_data and "preferences" in user_data[st.session_state.username]:
            if "keywords" in user_data[st.session_state.username]["preferences"]:
                if user_data[st.session_state.username]["preferences"]["keywords"]:
                    # Gunakan fungsi get_recommendations untuk mendapatkan rekomendasi
                    ...
                    st.info("Silakan atur preferensi Anda di tab Preferensi untuk mendapatkan rekomendasi.")
            else:
                st.info("Silakan atur preferensi Anda di tab Preferensi untuk mendapatkan rekomendasi.")
        else:
            st.info("Silakan atur preferensi Anda di tab Preferensi untuk mendapatkan rekomendasi.")
...
def show_navigation():
    with st.sidebar:
        ...
        # Tombol Home selalu ditampilkan
